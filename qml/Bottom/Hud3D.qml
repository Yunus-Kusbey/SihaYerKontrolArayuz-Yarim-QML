...
Canvas {
    ...
    onPaint: {
        ...
        // heading çizgileri yawDeg’e göre kayýyor
        for (let i = -6; i <= 6; i++) {
            let hdg = (yawDeg + i*10 + 360) % 360
            let x = cx + i * spacing
            ...
            // yazý
            ctx.fillText(hdg.toFixed(0).padStart(3,"0"), x - 10, cy + 15)
        }
    ...
    // heading deðerini sürekli artýrarak sonsuz döngü saðlýyoruz
    NumberAnimation on yawDeg {
        from: 0
        to: 360
        duration: 8000   // 8 saniyede tam tur
